...
class BlackjackApp {
  ...
  setupEventListeners() {
    document.addEventListener("tournamentStart", (e) =>
      this.startTournament(e.detail.enemyConfigs)
    );
  ...
  attachGameEventListeners() {
    document
      ...
      .addEventListener("click", () => this.playerHit());
    document
      .getElementById("stand-btn")
      .addEventListener("click", () => this.playerStand());
    document
      .getElementById("next-btn")
      .addEventListener("click", () => this.nextGame());
    document
      .getElementById("lobby-btn")
  ...
  startCurrentGame() {
    ...
    console.log(
      `Starting game: ${currentGame.player1.name} vs ${currentGame.player2.name}`
    );
  ...
  returnToLobby() {
    if (this.gameRoom && this.gameRoom.games.length > 0) {
      if (
        confirm("Return to lobby? Current tournament progress will be lost.")
      ) {
        this.showLobby();
      }
    } else {
  ...
  handleGameEnd() {
    ...
    console.log(
      "Scores before recording:",
      Array.from(this.gameRoom.scores.entries())
    );
  ...
  endTournament() {
    ...
    const tournamentDuration = Math.round(
      (Date.now() - this.tournamentStartTime) / 1000 / 60
    );
  ...
  renderCards(containerId, hand, isEnemy = false) {
    ...
    if (isEnemy && containerId === "player2-cards") {
      ...
      displayHand.forEach((card, index) => {
        ...
        const cardElem = DOMBuilder.createCardElement(
          card,
          isFaceUp,
          cardDesign
        );
  ...
  updateGameInfo(game) {
    ...
    if (currentPlayerElem)
      currentPlayerElem.textContent = game.currentPlayer.name;
