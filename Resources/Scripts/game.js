...
export class Game {
  constructor(player1, player2) {
    ...
    console.log(
      `Game created: ${this.player1.name} (${
        this.player1.isHuman ? "Human" : "AI"
      }) vs ${this.player2.name} (${this.player2.isHuman ? "Human" : "AI"})`
    );
  ...
  ensurePlayerObject(player) {
    ...
    // Fallback - create a human player
      ...
      console.warn(
        "Invalid player object, creating default human player:",
        player
      );
  ...
  startGame() {
    ...
    console.log(
      `Game started. Scores: ${this.player1.name}: ${this.player1.score}, ${this.player2.name}: ${this.player2.score}`
    );
  ...
  handleAITurn() {
    ...
    setTimeout(() => {
      ...
      if (
        this.currentPlayer.shouldHit &&
        !this.currentPlayer.isStanding &&
        !this.currentPlayer.isBust()
      ) {
        const shouldHit = this.currentPlayer.shouldHit();
        console.log(
          `AI ${this.currentPlayer.name} decision: ${
            shouldHit ? "HIT" : "STAND"
  ...
  hit() {
    ...
    console.log(
      `${this.currentPlayer.name} hits, current score: ${this.currentPlayer.score}`
    );
    this.currentPlayer.addCard(this.deck.drawCard());
    console.log(
      `${this.currentPlayer.name} new score: ${this.currentPlayer.score}`
    );
    this.triggerUIUpdate();
  ...
  stand() {
    ...
    console.log(
      `${this.currentPlayer.name} stands with score: ${this.currentPlayer.score}`
    );
  ...
  switchPlayer() {
    ...
    console.log(
      `Switched to ${this.currentPlayer.name}'s turn, isHuman: ${this.currentPlayer.isHuman}`
    );
    this.triggerUIUpdate();
  }
  ...
  determineWinner() {
    console.log(
      `Determining winner: ${this.player1.name}: ${this.player1.score}, ${this.player2.name}: ${this.player2.score}`
    );
  ...
  endGame(winner) {
    ...
    if (winner) {
      console.log(
        `Winner details - name: "${winner.name}", isHuman: ${winner.isHuman}`
      );
    } else {
  ...
  getEnemyHandForDisplay() {
    if (this.shouldShowEnemyCards()) {
      return this.player2.hand.map((card) => ({
        ...card,
        isHidden: false,
      }));
    } else {
  ...
  getEnemyScoreForDisplay() {
    if (this.shouldShowEnemyCards()) {
      ...
      const firstCardValue =
        this.player2.hand.length > 0 ? this.player2.hand[0].value : 0;
