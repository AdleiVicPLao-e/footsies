...
export class GameRoom {
    ...
    startRoundRobin() {
        ...
        for (let i = 0; i < this.players.length; i++) {
            for (let j = i + 1; j < this.players.length; j++) {
                ...
                if (!player1 || !player2) {
                    console.error(`Could not find player objects for: ${player1Name} or ${player2Name}`);
                    continue;
                }
                ...
                console.log(`Created game: ${player1Name} (${player1.cardDesign}) vs ${player2Name} (${player2.cardDesign})`);
            }
        }
    ...
    getLeaderboard() {
        const leaderboard = Array.from(this.scores.entries()).sort((a, b) => b[1] - a[1]);
